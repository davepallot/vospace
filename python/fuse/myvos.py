"""A set of Python Classes for connecting to and interacting with a VOSpace
   ...
   Modified from the CADC version to work with a general VOSpace service.
"""
...
SERVER = os.getenv('VOSPACE_WEBSERVICE', 'dldev1.tuc.noao.edu:8080/vospace-2.0/vospace')
VOSPACE_GMS_PREFIX = "ivo://datalab.noao.edu/gms#"
VOSPACE_ARCHIVE = os.getenv("VOSPACE_ARCHIVE", "vospace")

HEADER_DELEG_TOKEN = 'X-VOSpace-DelegationToken'
...
class Connection:
    ...
    def __init__(self, security_token = None, http_debug=False): 
        """Setup the security for later usage
        
        security_token -- placeholder for arbitrary security token
        http_debug -- set True to generate httplib debug statements
        """
        self.security_token = security_token
        self.http_debug = True # http_debug MJG
    ...
    def get_connection(self, url):
        ...
        """
        parts = URLparse(url)
        logger.debug("Trying to connect to %s://%s using %s" % (parts.scheme, parts.netloc, self.security_token))
...
#        ElementTree.ElementTree(self.node).write(file, encoding="UTF-8")
    ...
    def chwgrp(self, group):
        ...
        if (group != None) and (group.count(VOSPACE_GMS_PREFIX) > 3):
            raise AttributeError("Exceeded max of 4 write groups: " +
                                 group.replace(VOSPACE_GMS_PREFIX, ""))
...
class VOFile:
    ...
    def close(self):
        ...
        try:
            ...
            self.closed = True
#           MJG 12/10/14 - closing the connection here wipes the response buffer - is this an issue
#           with the version of httplib being used?
#            self.httpCon.close()
        return self.closed

    def checkstatus(self, codes=(200, 201, 202, 206, 302, 303, 503, 416,
                                 416, 402, 408, 412, 504, 204)): # MJG Added 204
        ...
        if self.resp.status not in codes:
            ...
            if self.resp.status in VOFile.errnos.keys():
                ...
                if self.resp.status == 401 and self.connector.security_token is None:
    ...
    def open(self, URL, method="GET", bytes=None, possible_partial_read=False):
        ...
        if self.connector.security_token:
            self.httpCon.putheader(HEADER_DELEG_TOKEN,
                                   self.connector.security_token)
        ...
        if method in ["PUT", "POST", "DELETE"]:
            # Workaround for UWS library issues MJG 12/11/14
            ...
            if method == "PUT":
                ...
                if ext in ['.fz', '.fits', 'fit']:
                    ...
                    if contentType is None: contentType = "text/xml" # MJG 12/18/14 to create dirs
...
class Client:
    ...
    def __init__(self, rootNode=None, conn=None,
                 http_debug=False, security_token = None):
        ...
        security_token: arbitrary security token
        """

        if not isinstance(conn, Connection):
            conn = Connection(http_debug=http_debug)
        ...
        if node is None:
            ...
            with self.nodeCache.watch(uri) as watch:
                ...
                if uri[0:4] == 'vos:':
                    ...
                    logger.debug("HERE %s" % dom.getroot())
...
#            do_shortcut = self.cadc_short_cut
        ...
        if (do_shortcut and ((method == 'GET' and
            ...
            headers = {"Content-type": "application/x-www-form-urlencoded",
                       "Accept": "text/plain"}
            # Add token to header if present
            if self.conn.security_token:
                headers[HEADER_DELEG_TOKEN] = self.conn.security_token
        ...
        if method == 'GET': # MJG - was applying to all URLs
          fields = {}
          ...
          URL = "%s://%s/nodes/%s%s" % (self.protocol, server,
                                              parts.path.strip('/'), data)
        else:
          URL = "%s://%s/nodes/%s" % (self.protocol, server, parts.path.strip('/'))
    ...
    def transfer(self, uri, direction, view="defaultview", cutout=None):
        ...
        views = {"defaultview": "%s#%s" % (Node.IVOAURL, "defaultview"),
                 "data": "ivo://datalab.noao.edu/vospace/view#data", # NOT SUPPORTED YET
                 "cutout": "ivo://datalab.noao.edu/vospace/view#cutout" # NOT SUPPORTED YET
        }
        ...
        view = views["defaultview"] # MJG - Force this for the time being
