...
def test_start_uws(h, resource, body):
  ...
  # Run job
  data = dict(PHASE = "RUN")
  resp, content = h.request(BASE_URI + '%s/%s/phase' % (resource, job.jobId), 'POST', urlencode(data), headers = headers)
...
def test_uws(h, resource, body, fail = False, summary = ''):
  ...
  if fail:
    ...
    assert summary in job.errorSummary 
  else:
...
class PushToVoSpaceTestCase(unittest.TestCase):
  ...
  def ptest_basic_push_to_vospace(self):
    """
  ...
  def ptest_push_to_vospace_new_node(self):
    """
  ...
  def ptest_push_to_vospace_container(self):
    """
    ...
    set_transfer_view(self.transfer, 'ivo://ivoa.net/vospace/core#votable')
    set_transfer_protocol(self.transfer, 'ivo://ivoa.net/vospace/core#httpput')
    self.handle_job(self.transfer, fail = True, summary = 'Data cannot be uploaded to a container')

  def ptest_push_to_vospace_reserved_uri(self):
    """
  ...
  def old_test_quick_push_to_vospace(self):
    """
  ...
  def ptest_push_to_vospace_with_unsupported_view(self):
    """
  ...
  def ptest_push_to_vospace_with_unsupported_protocol(self):
    """
    ...
    self.handle_job(self.transfer, fail = True, summary = 'The service supports none of the requested Protocols')

  def ptest_push_to_vospace_with_multiple_protocols(self):
    pass
...
class PullFromVoSpaceTestCase(unittest.TestCase):
  ...
  def old_test_quick_pull_from_vospace(self):
    """
    Test transferring data from the space: node12
    """
